...
const MoneyLoan = ({ money, periods, name }) => {
    ...
    const exportToExcel = async () => {
        ...
        const worksheet = workbook.addWorksheet("รายงานยอดสะสมเงินกู้ยืม");
        ...
        worksheet.columns = [
            ...
            { header: "เบิกเงินกู้ยืม", key: "deduct", width: 20 },
            { header: "คืนเงินกู้ยืม", key: "return", width: 20 },
        ...
        titleCell.value = "รายงานยอดสะสมเงินกู้ยืม";
        ...
        money.forEach((row, index) => {
            ...
            // คำนวณยอดสะสม
            row.Type === "รายได้" ? cumulative += Number(row.Money || 0) : cumulative -= Number(row.Money || 0);
        ...
        saveAs(
            new Blob([buffer]),
            `รายงานยอดสะสมเงินกู้ยืม_${dayjs().format("YYYYMMDD_HHmmss")}.xlsx`
        );
    };
    ...
    return (
        <React.Fragment>
            <Tooltip title={"กดเพื่อดูรายละเอียดยอดสะสมเงินกู้"} placement="left">
                <TableCell
                    sx={{
                        textAlign: "left",
                        cursor: "pointer",
